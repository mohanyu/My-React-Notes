...
const classComponentUpdater = {
  ...
  enqueueSetState(inst, payload, callback) {
    const fiber = getInstance(inst);  // 获取 当前fiber
    console.log('fiber', fiber); //sy-log
    const currentTime = requestCurrentTimeForUpdate();  // 当前时间
    const suspenseConfig = requestCurrentSuspenseConfig();
    const expirationTime = computeExpirationForFiber( // 过期时间
      currentTime,
      fiber,
      suspenseConfig,
    );

    const update = createUpdate(expirationTime, suspenseConfig);  // 创建更新
    // 参数
    update.payload = payload; // setState()中的更新对象 {count: 2}
    ...
    // setState接下来 和 render一样
    enqueueUpdate(fiber, update);
    scheduleWork(fiber, expirationTime);
  },
  // 1. setState是修改其中的部分状态，相当于Object. assign, 只是覆盖，不会减少原来的状态;

  // 2. replaceState 是完全替换原来的状态，相当于赋值，将原来的state替换为另一个对象，
  // 如果新状态属性减少，那么state中就没有 这个状态了。
  ...
  enqueueForceUpdate(inst, callback) {  // 强制刷新
    const fiber = getInstance(inst);
    const currentTime = requestCurrentTimeForUpdate();
    const suspenseConfig = requestCurrentSuspenseConfig();
    const expirationTime = computeExpirationForFiber(
