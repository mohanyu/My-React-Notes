...
export function updateContainer(
  ...
  const expirationTime = computeExpirationForFiber( // 计算过期时间
    currentTime,    
    current,       // 当前的 fiber
    suspenseConfig,
  );
  ...
  update.payload = {element}; // element 是 render第一个参数 JSX - Element
  ...
  if (callback !== null) {
    ...
    update.callback = callback; // callback 又给了 update(还在传递)
  }

  enqueueUpdate(current, update); // current： 当前fiber  | update： 更新
